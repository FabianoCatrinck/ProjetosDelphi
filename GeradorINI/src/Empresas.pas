...
begin
  if DM.ParametrosEmpresa.Locate('CodigoEmpresa', Empresa, []) then
...
procedure TTelaEmpresas.DBGridDblClick(Sender: TObject);
begin
  BotaoAlterar.Click;
end;
...
procedure TTelaEmpresas.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if DM.DadosEmpresa.State = dsBrowse then
    AjustarTela(5);
end;
